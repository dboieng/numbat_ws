...
![Numbat High Level Electrical Diagram](images/Numbat_High_Level_Diagram.png)
...
Given the operating current of 0.9A for each MG996R servo and a stall current of 2.5A the group resolved to connect the servos directly to the PCA9685 servo rail. If issues arise one of the contingencies below will be inacted. I note: most spot repos advise the MG996R servos are insufficent for operation for the spot micro dog, even the orginal ESP32 repo suggests to use 20kg servo's instead. If we upgrade to 20kg servos then the stall current would stay drop to 2.2A but may have an increased operating current and may require implementing one of the following contingencies:
